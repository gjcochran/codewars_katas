...
# D/A
# want to record to original indices of all odd numbers in an array
# then can record all odd numbers in a separate array, sort it and iterate using []= method through original array ie arr[index[0]] = odd_arr[0]

def sort_array(arr)
  indices = []
  arr.each_with_index { |el, idx| indices << idx if el.odd? }
  odds = arr.select { |el| el.odd? }.sort
  indices.each_with_index { |el, idx| arr[el] = odds[idx] }
  arr
end
